...
| **이름** | **회고 내용** |
| :---: |---|
| 조아름 | 이탈의 정의와 프로젝트 방향을 선정하기 위해 여러 논문과 기사들을 찾아보며 결론을 낼 수 있었고 프로젝트를 진행하면서 이 과정이 정말 중요하고, 필수적이라는 생각이 들었다. 그리고 방대한 양의 데이터로 전처리 과정부터 어려움을 겪었고, 컬럼도 많고 이탈 분석을 위해 새로운 변수를 생성하는 과정에서 놓치는 부분도 있었던 것 같아 다음에는 자세하게 머리로 정리를 먼저 하고 실수없이 진행해야겠다는 생각이 들었다.  |
| 정재훈 | 기업의 실제 데이터가 3100만건정도 되었는데 이렇게 큰 데이터를 다뤄본것이 처음이라, 앞서 생각하지 못한 문제들이 발생하여 시간이 많이 지연됐던것 같아서 아쉬웠고, 종속 변수와 컬럼 간의 개념, 이탈 목적에 대한 고민들을 심도 깊게 할 수 있어서 공부가 많이 됐던 것 같다. 또한 비즈니스 관점에서 대안을 고민할 수 있는 시간이, 실무에서 꼭 필요한 생각방식이라고 생각하고, 이러한 여러 관점에서 고민을 했던 것들이 나중에 큰 도움이 될 것 같다. |
| 정석원 | 데이터에 대한 분석을 하기 위해 다양한 논문과 자료를 검색하며 개요 및 근거를 단단히 하였고,  방대한 양의 데이터셋을 이용하여 모델학습을 진행 하면서 하드웨어적인 한계로 인해 학습시간이 딜레이 되는 점이 아쉬웠다. 하지만 해당 문제를 해결하기 위해 다양한 시도를 해보며 실력을 더 늘릴 수 있었던 시간이였다. |
| 김민준 | 데이터 전처리를 하면서 결측치와 컬럼의 특징을 제대로 파악을 하지 못해서 결측치를 지우거나 0으로 만들어 버리는 실수를 하면서 머신러닝 모델에 악영향을 끼칠뻔 했고 XGBoost 같은 경우에는 Label encoding 보다는 One - Hot encoding을 하는 것이 안정적으로 예측을 할 수 있다는 것을 체감을 했다. 이러한 실수들 덕분에 전처리 단계가 모델 성능에 얼마나 큰 영향을 주는지 확실히 알게 되었고, 다음 프로젝트에서는 결측치를 무작정 처리하기보다 의미를 먼저 분석하고 기준을 세운 뒤 처리해보려고 한다.| 
| 정준하 | 신규·전체 고객을 분리하여 학습했을 때 변수 중요도와 성능이 달라지는 과정을 통해 모델 구조 설계의 중요성을 체감했습니다. LightGBM 선정 과정에서도 단순 정확도가 아닌 재현율과 실무 활용 가능성을 기준으로 판단하며, 모델 학습이 단순 성능 경쟁이 아닌 비즈니스 의사결정 도구라는 점을 배운 프로젝트였습니다.|
