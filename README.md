# 🔖2nd_PROJECT_4TEAM

# **프로젝트 명 : 떡잎 쇼핑몰 구하기** 📚

## 💡**팀명 : (재)훈이는 못말려**

---

## 🌟 팀원 소개

| 이름 | 역할 | 깃허브 계정 (Link) |
| :---:| :--- | :--- |
| **조아름** |  | [areum117](https://github.com/areum117) |
| **정재훈** |  | [JeaHoon-J](https://github.com/JeaHoon-J) |
| **정석원** |  | [JeongSW123](https://github.com/JeongSW123) |
| **김민준** |  | [miin-jun](https://github.com/miin-jun) |
| **정준하** |  | [junhaj27](https://github.com/junhaj27) |

---

## 1. 프로젝트 개요

### 1.1 프로젝트 주제 선정 배경

|<**뉴스기사**>||
|:---|:---|
|||
|||

- 국내 패션소비 시장은 코로나 이후 일시적인 반등에도 불구하고 2019년 이전 수준을 간신히 회복한 뒤 저성장 국면에 머물러 있다. 소비자는 더 이상 감성적 마케팅보다 ‘지금 나에게 얼마나 유용한가’를 따져 지갑을 여는 양상을 보이고 있다.

- 중저가·초저가 온라인 플랫폼으로의 이동이 가속화되면서 글로벌 SPA 브랜드 H&M 역시 예외가 아니다. 한국 1호 매장인 명동점 폐점을 포함해 오프라인 매장 구조조정을 단행했으며, 수년간의 수익성 악화와 재고 부담, 경쟁사 대비 포지셔닝 약화 등으로 기존 고객 기반이 흔들리고 있다는 평가를 받고 있다. 그럼에도 실제로 어떤 고객이 어떤 패턴을 보이다가 이탈하는지는, 데이터 기반으로 충분히 분석되지 않았다.


<**출처**>

- https://www.ktnews.com/news/articleView.html?idxno=136611
- https://www.ktnews.com/news/articleView.html?idxno=131569
- https://dmcdt.tistory.com/15


---

### 1.2 프로젝트 주제 필요성

- 패션 산업은 오랫동안 ‘감과 경험’에 의존하는 산업으로 인식되어 왔고, 현업에서도 데이터 분석의 필요성은 인지하지만 이를 체계적으로 수행할 수 있는 역량이나 환경은 아직 충분하지 않다. 따라서 **H&M의 고객 행동 데이터를 바탕으로 고객이탈을 조사하고, 이탈 위험을 예측하는 모델을 구축하는 것이 목표이다.** 이를 통해 감으로만 알던 고객이탈을 데이터로 설명하고, 신규 가입자의 관리가 중요하다는 것을 시각화 하고, 패션 기업의 데이터 기반 판단의 필요성을 실증적으로 보여주고자 한다. 더 나아가 전체적인 패션 업계의 성장을 바라본다.

---

## 2.  **기술 스택** 🛠️

| **분류**| **기술/도구** |
|---|---|
| **언어** | ![Python](https://img.shields.io/badge/python-3670A0?style=for-the-badge&logo=python)     |
| **라이브러리** | ![NumPy](https://img.shields.io/badge/numpy-013243?style=for-the-badge&logo=numpy) ![Pandas](https://img.shields.io/badge/pandas-150458?style=for-the-badge&logo=pandas) ![Matplotlib](https://img.shields.io/badge/Matplotlib-ffffff?style=for-the-badge&logo=Matplotlib) <br> ![Seaborn](https://img.shields.io/badge/seaborn-0C5A5A?style=for-the-badge&logo=Seaborn) ![scikitlearn](https://img.shields.io/badge/scikitlearn-green?style=for-the-badge&logo=scikitlearnlogo=xgboost) ![imbalanced-learn](https://img.shields.io/badge/imbalanced--learn-FF6B6B?style=for-the-badge) ![Joblib](https://img.shields.io/badge/Joblib-2E8B57?style=for-the-badge)
| **협업 툴** | ![GitHub](https://img.shields.io/badge/github-121011?style=for-the-badge&logo=github) ![Git](https://img.shields.io/badge/git-F05033?style=for-the-badge&logo=git)|

---

## 3. WBS 📋

---

## 4. 데이터 선택 및 특징 🗃️

### 4.1 데이터 선택

-


<**출처**>

---

### 4.2 데이터 특징

---

## 5. 데이터 전처리 및 EDA

### 5.1 데이터 전처리

#### 5.1.1 결측치 처리

#### 5.1.2 이상치 처리

#### 5.1.3 데이터 그룹화

---

### 5.2. 최종 데이터 구조

#### **🔹주요 변수**

-

#### **🔹분석 타겟 컬럼**


---

### 5.3 상관관계 분석

---

### 5.4 이탈률 분포

---

## 6. 머신러닝 파이프라인 🔧

### 6.1 사용 변수

---

### 6.2 Scaling & Encoding

---


### 6.4 모델

#### 6.4.1 Model


## 7. 하이퍼 파라미터 조정 모델 성능 결과 🖨️
---

#### 7.1.1 성능 비교

---

## 8. 인사이트 🔦

### 8.1 이용자 이탈 방지 전략

---

## 9. 한계점 🧩

- 패션 즉 의류업은 개인적인 관점이 들어가며 브랜드별 혹은 트랜드에 민감하다는 특징이 있어서 해당 부분에 대한 전략이나 해결책 제시는 불가능하다.
- 각 브랜드의 구체적인 손익에 대한 프로모션은 고려가 불가능하다.(10% 할인을 해도 몇 퍼센트 이상 판매한다면 이득이다 등)

- 또한 설문·리뷰 등 정성 데이터가 부족해, 정량 분석만으로는 고객의 ‘왜 떠나는지(WHY)’를 완전히 설명하기 어렵다는 한계가 있습니다
---

## 10. 수행 결과 페이지 📌


### **팀원 한 줄 회고** 🧑‍💻
 
| **이름** | **회고 내용** |
| :---: |---|
| 조아름 | |
| 정재훈 | |
| 정석원 | |
| 김민준 | | 
| 정준하 | |
